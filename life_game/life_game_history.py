...
colors = ["white", "black"] # Цвета мертвой/живой клеток
...
# Структура для изменения оттенка серого
#next_color={"black":"violet","violet":"blue","blue":"green", "green":"yellow", "yellow":"orange",
#            "orange":"red", "red":"white", "white":"white"}
next_color={"black":"lightblue","lightblue":"lightgreen", "lightgreen":"yellow",
            "yellow":"tomato", "tomato":"white", "white":"white"}
...
def repaint_all():
    ...
    for x in range(nun_of_cells):
        for y in range(nun_of_cells):
            ...
            color_cell = canvas.itemcget(screen[x][y], "fill")
            color_cell = next_color[color_cell]
            #r, g, b = root.winfo_rgb(color_cell)
            #r, g, b = r // 256, g // 256, b // 256
            #print(color_cell)
            if cells[x][y] == 1:
                canvas.itemconfig(screen[x][y], fill=colors[cells[x][y]])
            else:
                canvas.itemconfig(screen[x][y], fill=color_cell)
    ...
    for x in range(nun_of_cells):
        for y in range(nun_of_cells):
            screen[x][y] = canvas.create_rectangle(x * size_of_cell, y * size_of_cell, x * size_of_cell + size_of_cell,
                                                   y * size_of_cell + size_of_cell, fill="white")
